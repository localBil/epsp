<?php 
  ...
  include('auth.php');
     ...
     if(isset($_POST['change'])) {
        ...
        $si = $conn->query("SELECT a.history_id,a.*,de.*,d.*,da.* FROM history_appointments as a LEFT JOIN departments as de ON a.history_department_id = de.departments_id LEFT JOIN doctors as d ON a.history_doctor_id = d.doctors_id LEFT JOIN dates as da ON a.history_day_id = da.dates_id WHERE a.history_doctor_id = '$doctor_id'");
     }else{
      $doctor_id = 1;
       $si = $conn->query("SELECT a.history_id,a.*,de.*,d.*,da.* FROM history_appointments as a LEFT JOIN departments as de ON a.history_department_id = de.departments_id LEFT JOIN doctors as d ON a.history_doctor_id = d.doctors_id LEFT JOIN dates as da ON a.history_day_id = da.dates_id WHERE a.history_doctor_id = '$doctor_id'");
     }
          ...
          <div class="col-md-12">
            <div class="card ">
              <div class="card-header">
                <h4 class="card-title">Rendez-vous' Historique</h4>
                  ...
                  <table class="table tablesorter" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody class="text-primary">
                       ...
                       <tr>
                           ...
                           <td><?php echo $wu['name'] ?></td>
                           <td><?php echo $wu['datename']?></td>
                           <td><?php echo $wu['history_date']?></td>
                           <td><i class="tim-icons icon-trash-simple text-danger trasher"  id="<?php echo $key['history_id']?>"></i></td>
                       </tr>
                     <?php } ?>
                    </tbody>
                  </table>
